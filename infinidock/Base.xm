...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)_scrollView {
    ...
    CGFloat leftOrigin = [self originForIcon:position].x;
...
- (void)restoreToPage {
    ...
    if (PAGING_ENABLED) {
        ...
        newOffset.x = [dock originForIconAtX:startIndex Y:0].x;
...
- (void)moveIconFromWindow:(SBIcon *)icon toIconList:(id)iconList {
    if (iconList == [[IFBase sharedInstance] dock]) {
        ...
        if (objc_getClass("SBIconView"))
            ic = [[objc_getClass("SBIconViewMap") homescreenMap] iconViewForIcon:icon];

        CGRect frame = [ic frame];
