...
- (int)columnAtPoint:(CGPoint)point {
    ...
    for (int i = 0; i < [[self icons] count]; i++) {
        ...
        if ([self originForIconAtIndex:i].x + (ICON_WIDTH / 2.0) > point.x)
            break;
...
- (CGRect)_contentViewRelativeFrameForIcon:(id)icon {
    ...
    id map = [objc_getClass("SBIconViewMap") homescreenMap];
    if (map != nil) {
        icon = [map iconViewForIcon:icon];
    }
...
__attribute__((constructor)) static void four_init() {
    ...
    if ([IFFirmwareVersion() hasPrefix:@"4.0"] || [IFFirmwareVersion() hasPrefix:@"4.1"] || [IFFirmwareVersion() hasPrefix:@"4.2"] || [IFFirmwareVersion() hasPrefix:@"4.3"] || [IFFirmwareVersion() hasPrefix:@"5.0"] || [IFFirmwareVersion() hasPrefix:@"5.1"]) {
        is42 = [IFFirmwareVersion() hasPrefix:@"4.2"] || [IFFirmwareVersion() hasPrefix:@"4.3"] || [IFFirmwareVersion() hasPrefix:@"5"];
        %init(Four);
